...
# 建立分頁標籤（第三個分頁已命名為 股市熱力圖）
tab1, tab2, tab3 = st.tabs(["📈 低檔打底選股", "🚨 處置股追蹤", "股市熱力圖"])

with tab3:
    st.title("🗺️ 股市熱力圖")
    st.markdown("💡 **操作說明**：請用滑鼠**左鍵點擊熱力圖中的任意股票方塊**，下方即會**立刻放大顯示該股票當日的日內分時走勢圖**！")
    ...
    if HAS_PLOTLY_EVENTS:
        ...
        st.warning("⚠️ 尚未安裝 `streamlit-plotly-events` 套件，請執行 `pip install streamlit-plotly-events` 以支援點擊熱力圖聯動。目前預設使用下方選單切換：")
